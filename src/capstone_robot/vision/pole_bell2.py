...
class PoleBellTracker2:
    ...
    def detect_pole_line(self, frame):
        ...
        line = choose_pole_line(candidates, angle_group_deg=self.angle_group_deg)
        if line is None:
            return None
...
def detect_pole_bell_alignment(frame, tracker=None, color_format="rgb"):
    ...
    bx, by, _ = bell
    pole_line = orient_line_toward_bell(pole_line, (bx, by))
    error = line_point_signed_distance(bx, by, pole_line)
