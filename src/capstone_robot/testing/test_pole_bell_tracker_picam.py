...
def draw_alignment(frame_rgb, alignment, status):
    vis = frame_rgb.copy()
...
def main():
    parser = argparse.ArgumentParser(description="Preview PoleBellTracker detections from the Pi camera.")
    parser.add_argument("--camera-index", type=int, default=0, help="Picamera2 camera index")
    ...
    print("Running PoleBellTracker test. Press Ctrl+C to stop.")
    if args.show:
    ...
    try:
        while True:
            ...
            if not ok or frame_rgb is None:
                print("[POLE TEST] No camera frame")
                time.sleep(0.05)
                continue
            ...
            alignment = tracker.detect(frame_rgb)
            frame_count += 1

            if alignment is None:
                stable_frames = 0
                status = "POLE TEST: NO ALIGNMENT"
            else:
                stable_frames = min(args.required_frames, stable_frames + 1)
                status = f"POLE TEST: TRACKING {stable_frames}/{args.required_frames}"

            elapsed = max(1e-6, time.time() - started_at)
            fps = frame_count / elapsed
            status = f"{status}  fps={fps:.1f}"

            vis = draw_alignment(frame_rgb, alignment, status)

            if preview is not None:
                preview.update(resize_preview(vis, args.preview_width))

            if args.show:
                cv2.imshow("PoleBellTracker Pi camera test", vis)
